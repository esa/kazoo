...
cd "$SKELS" && rm -f gr740.zip && zip -r gr740 gr740/* && cd $OLDPWD || exit -1

cd "$SKELS" && rm -f pc.zip && zip -r pc pc/* && cd $OLDPWD || exit -1
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subC gr740:"$SKELS"/gr740.zip \
	--subC pc:"$SKELS"/pc.zip \
